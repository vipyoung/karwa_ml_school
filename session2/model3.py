...
def read_data_distance(fname):
    ...
    with open(fname) as f:
        ...
        for line in f:
            ...
            s_zone = int(x[9])
            d_zone = int(x[10])
            #data.append([hour_of_day+1, day_of_week+1, osrm_dur, osrm_dist, krw_dur])
            # data.append([osrm_dist, krw_dur])
            data.append([hour_of_day+1, day_of_week+1, day_of_week*24+(hour_of_day+1), osrm_dist, osrm_dur, s_zone, d_zone, krw_dur])
    return data
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(X):
        ...
        mod = GradientBoostingRegressor(n_estimators=10, max_depth=5)
